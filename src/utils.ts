...
import {
    CANVAS_PADDING, CARD_HEIGHT, CARD_WIDTH, OPEN_PADDNIG, PADDING, RANKS, SUITS
} from './constants';
...
export function cardPileIntersect(card: Card, pile: Pile): boolean {
  ...
  const pileBox = {
    ...
    y: pile.getBounds().y,
    height: pileLastChild().getBounds().y + pileLastChild().getBounds().height,
  };
...
export function calcCanvasHeight(): number {
  return (
    CANVAS_PADDING * 2 + CARD_HEIGHT + PADDING + OPEN_PADDNIG * 13 + CARD_HEIGHT
  );
}
